...
package com.amap.api.maps2d.heatmaps;
...
import com.amap.api.maps2d.geometry.Bounds;
import com.amap.api.maps2d.geometry.Point;
import com.amap.api.maps2d.model.LatLng;
import com.amap.api.maps2d.model.Tile;
import com.amap.api.maps2d.model.TileProvider;
import com.amap.api.maps2d.quadtree.PointQuadTree;
...
public class HeatmapTileProvider implements TileProvider {
	...
	public Tile getTile(final int x, final int y, final int zoom) {
		...
		final double tileWidth = WORLD_WIDTH / Math.pow(2, zoom);

		// how much padding to include in search
		// is to tileWidth as mRadius (padding in terms of pixels) is to
		// TILE_DIM
		// In terms of world width units
		final double padding = tileWidth * mRadius / TILE_DIM;
		...
		// In terms of world width units
		final double bucketWidth = tileWidthPadded / (TILE_DIM + mRadius * 2);
		...
		if (minX < 0) {
			...
			final Bounds overlapBounds = new Bounds(minX + WORLD_WIDTH, WORLD_WIDTH, minY, maxY);
			xOffset = -WORLD_WIDTH;
			...
			final Bounds overlapBounds = new Bounds(0, maxX - WORLD_WIDTH, minY, maxY);
			xOffset = WORLD_WIDTH;
			wrappedPoints = mTree.search(overlapBounds);
		}
		...
		final Bounds paddedBounds = new Bounds(mBounds.minX - padding, mBounds.maxX + padding, mBounds.minY - padding,
				mBounds.maxY + padding);
		if (!tileBounds.intersects(paddedBounds)) return TileProvider.NO_TILE;
		...
		if (points.isEmpty()) return TileProvider.NO_TILE;

		// Quantize points
		final double[][] intensity = new double[TILE_DIM + mRadius * 2][TILE_DIM + mRadius * 2];
		...
		final double[][] convolved = convolve(intensity, mKernel);

		// Color it into a bitmap
		final Bitmap bitmap = colorize(convolved, mColorMap, mMaxIntensity[zoom]);
	 ...
	 * clear overlay's tile cache (using clearTileCache()) after calling this.
	 * 
	 * @param data Data set of points to use in the heatmap, as LatLngs. Note:
	...
	public void setWeightedData(final Collection<WeightedLatLng> data) {
		...
		if (mData.isEmpty()) throw new IllegalArgumentException("No input points.");
	...
	private double[] getMaxIntensities(final int radius) {
		...
		final double[] maxIntensityArray = new double[MAX_ZOOM_LEVEL];
	 ...
	 */
	private static Collection<WeightedLatLng> wrapData(final Collection<LatLng> data) {
	 ...
	 * cropped of padding.
	 * 
	 * @param grid Raw input grid to convolve: dimension (dim + 2 * radius) x
	...
	public static class Builder {
		 ...
		 * 
		 * @param val Collection of LatLngs to put into quadtree. Should be
		 *            non-empty.
		...
		public Builder opacity(final double val) {
			...
			if (opacity < 0 || opacity > 1) throw new IllegalArgumentException("Opacity must be in range [0, 1]");
			return this;
		}
		 ...
		 * 
		 * @param val Radius of convolution to use, in terms of pixels. Must be
		 ...
		 * 
		 * @param val Collection of WeightedLatLngs to put into quadtree. Should
		 *            be non-empty.
		 * @return updated builder object
		 */
		public Builder weightedData(final Collection<WeightedLatLng> val) {
			...
			if (data.isEmpty()) throw new IllegalArgumentException("No input points.");
			return this;
		}
	}
}
